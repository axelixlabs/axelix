---
title: Beans
description: "The Beans page lists every Spring bean registered in the managed instance's ApplicationContext. Each row is an
...
![beans main page](/img/feature/beans/beans-main-page.png) ***Beans page as presented in Axelix UI***
...
![beans dropdown page](/img/feature/beans/beans-details-page.png) ***Bean dropdown page as presented in
Axelix UI***
...
- **`@Component` or variants** — the bean came from `@Component` / `@Service` / `@Repository` / `@Controller` / custom
  stereotype. When the declaring class is tracked on the [Conditions](./conditions.mdx) page, the label switches to
...
- **`@Bean` method** — the bean came from a `@Bean` factory method. The tree shows:
    ...
    - **Declaring class** — the class that defines the `@Bean` method, clickable to jump to the bean whose class
      ...
      class is tracked on the [Conditions](./conditions.mdx) page.
    - **Who defines this bean?** link to the [Conditions](./conditions.mdx) page — shown for the same case, i.e. when the
      declaring class is tracked there.
    - **`@ConfigurationProperties` bean** link to the [Configuration Properties](./configuration-properties.mdx) page (only
...
- [Configuration Properties](./configuration-properties.mdx) — reached from the **`@ConfigurationProperties` bean** link in
  Origin.
- [Conditions](./conditions.mdx) — reached from the **`@AutoConfiguration class`** / **Who defines this bean?** links in
  Origin.
