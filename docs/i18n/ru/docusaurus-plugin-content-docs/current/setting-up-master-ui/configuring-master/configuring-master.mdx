...
`axelix.master.monitoring.prometheus.enabled=true`. Он отвечает по адресу `/api/actuator/prometheus` и не требует
аутентификации.

Добавляйте общие теги к каждой экспортируемой метрике через `axelix.master.monitoring.prometheus.tags.<name>=<value>`,
...
| Свойство                                      | По умолчанию | Описание                                                                        |
|------------------------------------------------|--------------|--------------------------------------------------------------------------------------|
| `axelix.master.monitoring.prometheus.enabled` | `false`      | Открыть эндпоинт сбора метрик Prometheus по адресу `/api/actuator/prometheus`.  |
| `axelix.master.monitoring.prometheus.tags`    | empty        | Общие теги, применяемые к каждой экспортируемой метрике.                       |
