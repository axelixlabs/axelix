...
| `axelix.master.auth.options.oauth2.state-required`      | `false`                                                       | Требовать и проверять значение `state` для защиты от CSRF в потоке авторизации с использованием кода. См. [Аутентификация](../authentication/authentication.mdx#oauth2--oidc). |
