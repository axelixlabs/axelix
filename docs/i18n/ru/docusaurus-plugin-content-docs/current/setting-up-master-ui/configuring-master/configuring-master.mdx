...
**Переменные окружения** — удобно, когда ваша оболочка или менеджер процессов уже экспортирует значения:
...
**Системные свойства JVM (`-D`)** — удобно для быстрого разового запуска:
...
**Отдельные переменные окружения (рекомендуется)** — имена в форме relaxed-binding из раздела [Способы задания
свойств](#способы-задания-свойств), переданные через `-e`. Именно их использует команда запуска выше:
...
**Системные свойства `-D` через `JAVA_TOOL_OPTIONS`** — JVM автоматически применяет эту переменную к команде `java`,
поэтому она удобна как хук для настройки самой JVM, например размера кучи. Используйте её только для несекретных
значений:
