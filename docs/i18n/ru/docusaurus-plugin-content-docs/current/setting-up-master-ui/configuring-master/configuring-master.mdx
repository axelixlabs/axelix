...
Трёх источников выше хватает для большинства случаев, но нередко конфигурацию Axelix Master хочется задать из внешнего
файла YAML или properties — например, из файла, смонтированного в контейнер по нестандартному пути.
...
Поскольку это свойство определяет, *откуда* читается конфигурация, оно должно быть известно до того, как это чтение
произойдёт. Задавайте его через **системное свойство JVM** или **переменную окружения** — но не изнутри того самого
файла, на который оно указывает:
...
Если у вас уже развёрнут [Spring Cloud Config Server](https://docs.spring.io/spring-cloud-config/reference/server.html)
...
Как и свойство расположения файла выше, эти свойства определяют, *откуда* читается конфигурация, поэтому должны быть
известны до начала чтения. Задавайте их через **системные свойства JVM** или **переменные окружения** — но не из файла,
который сам же отдаёт Config Server. Минимальный запуск выглядит так:
...
:::warning
Когда функция включена, Master стартует в режиме fail-fast. Если Config Server недоступен при запуске, Master не
загрузится, вместо того чтобы стартовать с неполной конфигурацией. Пока функция выключена (по умолчанию), Master не
обращается к серверу и запускается штатно.
:::
...
Если ваша организация хранит секреты в [HashiCorp Vault](https://developer.hashicorp.com/vault), Master может читать
оттуда и свою конфигурацию. При старте Master получает секрет из
[движка секретов KV](https://developer.hashicorp.com/vault/docs/secrets/kv) и подмешивает каждый ключ этого секрета к
...
Внутри Master выступает клиентом [Spring Cloud Vault](https://docs.spring.io/spring-cloud-vault/reference/). Поэтому под
...
Vault читается **один раз, при старте**. Изменённый секрет вступает в силу после перезапуска Master, и читается только
статический движок KV — динамические движки секретов, например короткоживущие учётные данные БД, в эту функцию не
входят.
...
избавить, поэтому в продакшене используйте [AppRole](#аутентификация-через-approle) или [Kubernetes](#аутентификация-через-kubernetes) ниже.
...
[AppRole](https://developer.hashicorp.com/vault/docs/auth/approle) — метод «машина-машина» для сред без платформенной
...
`secret-id` сам по себе является секретом, поэтому на контейнерных платформах передавайте его переменной окружения
(`AXELIX_MASTER_EXTERNALCONFIG_SPRINGCLOUDVAULT_APPROLE_SECRETID`), а не флагом `-D`.

#### Аутентификация через Kubernetes

В Kubernetes мы рекомендуем [метод аутентификации Kubernetes](https://developer.hashicorp.com/vault/docs/auth/kubernetes):
Master аутентифицируется токеном сервис-аккаунта, который и так есть у его пода, так что статических учётных данных не
существует вовсе. На стороне Vault создайте роль в бэкенде аутентификации Kubernetes и привяжите её к сервис-аккаунту
Master. На стороне Master укажите имя этой роли:
...
Как и в соседних разделах выше, все эти свойства определяют, *откуда* читается конфигурация, поэтому должны быть
известны до начала чтения: задавайте их через системные свойства JVM или переменные окружения — но не ключами внутри
секрета Vault.

:::warning
Когда функция включена, Master стартует в режиме fail-fast. Если Vault недоступен при запуске, Master не загрузится,
вместо того чтобы стартовать с неполной конфигурацией. Пока функция выключена (по умолчанию), Master не обращается к
Vault и запускается штатно.
:::
...
Возьмите JAR-файл Axelix Master, прикреплённый к нужному вам релизу, со [страницы
...
одинаково (см. [Способы задания свойств](#способы-задания-свойств)): через **переменные окружения** или **системные
...
Master](#указание-файла-конфигурации-для-master).
...
Предпочитайте **переменные окружения** — внутри контейнера это самое безопасное место для конфигурации и единственный
вариант, при котором секреты не попадают в стартовый лог.
...
свойств](#способы-задания-свойств), передаются они через `-e`. Именно их использует команда запуска выше:
...
Master](#указание-файла-конфигурации-для-master)). Не держите секреты в файле, встроенном в образ или закоммиченном
...
Каждый ключ `AXELIX_MASTER_*` сопоставляется с соответствующим свойством через relaxed binding (см. [Способы задания
свойств](#способы-задания-свойств)). Передача их отдельными переменными окружения — а не через `JAVA_TOOL_OPTIONS` —
...
`AXELIX_MASTER_AUTH_COOKIE_SECURE` в этом примере `false`, поскольку фрагмент кода предоставляет доступ к Master по
...
конфигурации для Master](#указание-файла-конфигурации-для-master)):
...
Не держите секреты — ключ подписи JWT, пароль суперадминистратора, учётные данные базы данных — в закоммиченном файле;
передавайте их отдельными переменными окружения, как показано выше.
