...
Оставьте в списке и `health`. Задание `management.endpoints.web.exposure.include` **заменяет** значение по умолчанию
Spring Boot (`health`), а стартеру необходим бин `HealthEndpoint` — через него он читает состояние приложения и
передаёт статус в Master в составе `axelix-metadata`. Если эндпоинт `health` не открыт нигде (ни через веб, ни через
JMX), Spring Boot не создаст этот бин, и приложение не запустится.

Добавьте также нативный эндпоинт `info`: `axelix-details` активируется только тогда, когда доступен `info` (в силу просто разных
исторических причин).
...
```properties
management.endpoints.web.exposure.include=health,info,axelix-metadata,axelix-beans,axelix-caches,axelix-conditions,axelix-configprops,axelix-details,axelix-env,axelix-feign,axelix-gc,axelix-heap-dump,axelix-loggers,axelix-metrics,axelix-scheduled-tasks,axelix-thread-dump,axelix-transactions-monitoring
```
