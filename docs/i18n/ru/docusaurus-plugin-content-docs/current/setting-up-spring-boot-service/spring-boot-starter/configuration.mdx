...
Стандартные эндпоинты `health` и `info` Axelix больше не требуются, поэтому стартер сам их не открывает. Открытыми
будут только эндпоинты, перечисленные вами в `management.endpoints.web.exposure.include`, плюс эндпоинты Axelix —
добавьте `health` и `info` в список сами, когда они вам нужны, в том числе `health`, который Spring Boot иначе открыл
бы по умолчанию.
