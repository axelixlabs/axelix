...
Ничего настраивать вручную не нужно. Стартер сам добавляет свои actuator-эндпоинты в
...
Стандартные эндпоинты `health` и `info` Axelix больше не требуются, поэтому стартер их не навязывает. Пока вы не задаёте
`management.endpoints.web.exposure.include` сами, стартер сохраняет `health`, который Spring Boot открывает по
умолчанию. Если же вы задаёте это свойство, открытыми будут только перечисленные вами эндпоинты плюс эндпоинты Axelix —
добавьте `health` и `info` в список сами, когда они вам нужны.
