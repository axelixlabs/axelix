...
Это разные Maven-артефакты, поставляемые отдельно (хотя все они следуют одной схеме версионирования). Поэтому при
добавлении Spring Boot стартера нужно убедиться, что вы добавляете стартер под соответствующую мажорную версию Spring Boot.
...
Вы запускаете Axelix Master локально, добавляете стартер в своё приложение и запускаете его из IDE. Приложение стартует без
...
На версиях Axelix ниже 1.2.0 (не включительно) нужно ещё одно свойство — старые стартеры не открывают свои эндпоинты
...
стартер](../setting-up-spring-boot-service/spring-boot-starter/configuration.mdx#что-открывает-стартер).
