...
open it and trigger the **Download state** export. Note that the environment-properties and configuration-properties
files inside the exported archive are still subject to the per-page authority masking: callers without
`ENV_VALUES_READ` / `CONFIG_PROPS_VALUES_READ` get `******` for those values. See
