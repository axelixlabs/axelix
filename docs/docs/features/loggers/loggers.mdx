...
### Loggers
For example, the logger `com.axelixlabs.axelix.sbs` has a logging level of <INFO /> (starting point).
...
Note that when changing the logging level of `com.axelixlabs.axelix` from <INFO /> to <DEBUG /> (step 2),
it will also be marked with an icon <TargetThemeIcon />, and all loggers with the prefix `com.axelixlabs.axelix`
will change their level to <DEBUG /> (step 2). However, they will not be marked with an icon <TargetThemeIcon />,
because their level matches the parent logger `com.axelixlabs.axelix`.
