...
Spring Boot hides custom actuator endpoints from web exposure by default. Add the Axelix IDs to `management.endpoints.web.exposure.include` so Master can reach them. `axelix-metadata` is the one Master probes first to confirm the instance is alive; the rest power the corresponding features in the UI.
...
Every Axelix actuator endpoint is gated by `JwtAuthorizationFilter`. The filter expects an HS256/HS512 token signed with the same key Master uses, so the two sides must agree on the algorithm and the secret. Use the same value you set for `axelix.master.auth.jwt.signing-key` on Master.
...
|---------------------------------------|---------|------------------------------------------------------------------------------------------------------------------------------|
| `axelix.sbs.auth.jwt.algorithm`       | *(unset)* | Signing algorithm. One of `HMAC256` or `HMAC512`. Must match Master's `axelix.master.auth.jwt.algorithm`.                  |
| `axelix.sbs.auth.jwt.signing-key`     | *(unset)* | Shared secret used to verify Master-issued tokens and to sign self-registration heartbeats. Must match Master's signing key. |
...
| `axelix.sbs.discovery.master-url`              | *(unset)* | Full URL of Master's self-registration endpoint, e.g. `http://master:8080/api/internal/service/register`. Required when `auto=true`. |
| `axelix.sbs.discovery.instance-url`            | *(unset)* | Base URL of this service as reachable from Master (no `/actuator` suffix). Required when `auto=true`.                        |
| `axelix.sbs.discovery.instance-name`           | *(unset)* | Display name shown in Axelix UI and MCP responses. Required when `auto=true`.                                                |
...
- [Authentication](../setting-up-master-ui/authentication.md) — deeper look at JWT and authority resolution on both sides.
