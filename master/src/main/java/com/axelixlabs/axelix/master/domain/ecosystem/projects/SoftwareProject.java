 ...
 * entry owns all three sets of {@link #libraries()}. Curating per artifact would duplicate the prose across every
 ...
 * @param libraries   every library the project publishes that a managed application might resolve; never empty
    ...
    public SoftwareProject {
        if (libraries.isEmpty()) {
            throw new IllegalArgumentException(
                    "Library '%s' declares no libraries, so nothing could ever match it".formatted(id));
        }
