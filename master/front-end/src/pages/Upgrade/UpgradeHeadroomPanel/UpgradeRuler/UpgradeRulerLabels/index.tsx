...
export const UpgradeRulerLabels = ({ ticks }: IProps) => {
    return (
        <>
            <div className={styles.MainWrapper}>
                {ticks.map(({ version, annotation, isSupported }) => (
                    <div className={styles.RulerTick} key={version}>
                        ...
                        {annotation && (
                            <span className={`TextUltraSmall ${styles.TickAnnotation}`}>
                                {annotation === "oldestSeen" && "oldest seen"}
                                {annotation === "masterNow" && "Master now"}
                                {annotation === "safeNext" && "+1 · safe"}
                                {annotation === "drop" && "+2 · drops 5"}
                            </span>
                        )}
                    </div>
                ))}
