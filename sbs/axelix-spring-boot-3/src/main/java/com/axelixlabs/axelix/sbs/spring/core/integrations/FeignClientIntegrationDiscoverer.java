...
public class FeignClientIntegrationDiscoverer implements IntegrationComponentDiscoverer<FeignIntegration> {
    ...
    private @Nullable String pickPath(String[] path, String feignPath) {
        if (path.length > 0) {
            return environment.resolvePlaceholders(feignPath) + environment.resolvePlaceholders(path[0]);
        }

        return !feignPath.isBlank() ? environment.resolvePlaceholders(feignPath) : null;
    }
}
