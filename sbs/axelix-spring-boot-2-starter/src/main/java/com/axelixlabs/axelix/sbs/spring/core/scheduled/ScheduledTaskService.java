...
public final class ScheduledTaskService {
     ...
     */
    public void modifyInterval(String taskId, long newInterval) {
        try {
            ManagedScheduledTask task = registry.findRequired(taskId);
    ...
    private IntervalTask recreateIntervalTask(Task task, long interval) {
        if (task instanceof FixedDelayTask) {
            FixedDelayTask t = (FixedDelayTask) task;
            return new FixedDelayTask(t.getRunnable(), interval, t.getInitialDelay());
        }
        if (task instanceof FixedRateTask) {
            FixedRateTask t = (FixedRateTask) task;
            return new FixedRateTask(t.getRunnable(), interval, t.getInitialDelay());
        }
